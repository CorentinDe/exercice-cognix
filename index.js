...
const endpoint = 'https://www.reddit.com/r/aww.json?sort=new';
...
//          -> Les titres de chaque post sont modifiés pour être passés en majuscule, la fonction printPostsSidebar
//          -> étant appelée avant printPosts ils sont en majuscules également dans celle ci. Autant faire le passage en majuscule à l'affichage dans ce cas.
function printPostsSidebar(redditPosts) {
    ...
    const posts = redditPosts.map((item, index) => {
        return `
            <li>
                <a href='${item.data.url}'>
                    <h6>${item.data.title.toUpperCase()}</h6>
                </a>
            </li>
        `;
    })
